...
void myWidthColorLine(int x1, int y1, int x2, int y2, int color1, int color2, int color3, int width) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
...
bool isCursorKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	key = GetAsyncKeyState(VK_RETURN); // To check if enter is pressed
	if (key == 1)
	{
		whichKey = 5;
		return true;
